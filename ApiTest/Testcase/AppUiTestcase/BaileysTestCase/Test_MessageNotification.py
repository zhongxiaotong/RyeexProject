#!/usr/bin/python
# -*- coding: utf-8 -*-
# @Time : 2021/7/15 11:29
# @Author : Greey
# @FileName: Test_MessageNotification.py
...
@allure.description('1：发送应用消息（微信）提醒；2：检查是否有消息提醒页面；3：发送应用消息（facebook）提醒；4：检查是否有消息提醒页面')
class TestClass:
    def setup(self):
        print("Test Start")
        self.log = MyLog()
    ...
    @allure.title("应用消息（微信，facebook）提醒")
    @allure.story("正常流程")
    ...
    def test_messagenotification(self):
        ...
        self.driver.keyevent(4)
        self.app.devices_click('SATURN_APP')
        self.app.tv_send_notification('{"appMessage": {"appId": "app.facebook", "text": "ryeex", "title": "ryeex"}, "type": "APP_MESSAGE"}')
        self.driver.keyevent(4)
        self.app.devices_click('SATURN_设备')
        self.app.assert_getdevicepagename("remind", "view_app_notify")
        self.app.device_home()
